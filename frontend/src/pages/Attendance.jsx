    ...
    <div className="space-y-6 p-4">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border flex flex-wrap gap-4 items-center justify-between">
        <div className="flex gap-4 items-center">
          <select 
            className="p-3 border rounded-xl w-48 font-medium outline-none focus:ring-2 focus:ring-indigo-500"
        ...
        <div className="flex gap-3">
          <button onClick={exportToCSV} className="bg-emerald-50 text-emerald-700 px-5 py-3 rounded-xl font-bold flex items-center gap-2 border border-emerald-100 hover:bg-emerald-100 transition shadow-sm">
            <FileDown size={20} /> Export
          </button>
          <button onClick={markAllPresent} disabled={!selectedGrade} className="bg-green-600 text-white px-5 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-green-700 transition shadow-lg shadow-green-100 disabled:bg-gray-300">
            <Check size={20} /> Mark All
          </button>
          <button onClick={() => {if(window.confirm("Clear all marks for this month?")) setAttendanceRecords({})}} className="bg-red-500 text-white px-5 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-red-600 transition shadow-lg shadow-red-100">
            <Trash2 size={20} /> Clear All
          </button>
          <button onClick={submitAttendance} disabled={saving || !selectedGrade} className="bg-indigo-600 text-white px-5 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-indigo-700 transition shadow-lg shadow-indigo-100 disabled:bg-gray-300">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y">
              {filteredStudents.map(student => (
                <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-4 text-center font-bold text-indigo-600 bg-indigo-50/30 sticky right-0 z-10 border-l">
            ...
            <tfoot className="bg-gray-100 font-bold border-t">
              <tr>
                ...
                {daysArray.map(day => (
                  <td key={day} className="text-center text-indigo-600 border-r">{getDailyCount(day)}</td>
                ))}
